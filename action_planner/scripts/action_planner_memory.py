...
class ActionPlannerMemory:
    ...
    def init(self, domain_file, problem_file, logger):
        self.logger = logger

        self.get_logger().info("Initializing ActionPlannerMemory...")
        if not self.set_domain_and_problem(domain_file, problem_file):
            self.get_logger().error("Failed to set domain and problem.")
            return False
    ...
    def set_domain_and_problem(self, domain_file, problem_file):
        self.get_logger().info(f"Setting domain '{domain_file}' and problem '{problem_file}'")
        reader = PDDLReader()
        try:    
            self.pddl = reader.parse_problem(domain_file, problem_file)
        except Exception as e:
            self.get_logger().error(f"Error while setting domain and problem: {e}")
            return False
    ...
    def _are_conditions_met(self, at_start_conditions):
        ...
        for condition in at_start_conditions:
            # Check if it's an AND expression and decompose it
            if hasattr(condition, 'node_type') and condition.node_type.name == 'AND':
                # Recursively check all sub-conditions
                if not self._are_conditions_met(condition.args):
                    return False
            elif hasattr(condition, 'node_type') and condition.node_type.name in {
                'GT', 'GE', 'LT', 'LE', 'EQUALS'
            }:
                ...
                if not satisfied:
                    self.get_logger().info(f"Numeric condition NOT satisfied: {condition}")
                    return False
            else:
                value = current_state.get(condition, False)
                value = str(value)=="true" # convert to boolean
                if not value:
                    self.get_logger().error(f"Condition NOT met: {condition}")
                    return False
    ...
    def check_conditions_action(self, action_name, parameters, timings):
        ...
        # Check if it's a durative action or instantaneous action
        is_durative = hasattr(action, 'duration')
        
        if is_durative:
            # Durative actions have conditions with timings
            at_start = []
            at_end = []
            over_all = []
            ...
            # Instantaneous actions have simple preconditions
            conditions = list(action.preconditions)
        ...
        self.get_logger().info(f"All conditions met: {all_met}")
        return all_met
    ...
    def apply_effects(self, action_name, parameters, timings):
        ...
        if is_durative:
            # Durative actions have timed effects
            temp = []
            if "at start" in timings:
                temp.append("start")
            if "at end" in timings:
            ...
            # Instantaneous actions have simple effects
