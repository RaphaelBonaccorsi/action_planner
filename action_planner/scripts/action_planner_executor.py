    ...
    def check_if_can_start_action(self):
        ...
        for action in self.current_plan:
            if action['state'] == ActionState.PENDING:
                ...
                self.get_logger().info(f"Checking if action {action['name']} {action['args']} can be started...")
                if self.memory.check_conditions_action(action['name'], action['args'], ["at start", "over all"]):
                    ...
                    self.get_logger().error(f"action {action['name']} {action['args']} cannot be started, conditions not met")
