...
class ActionPlanner(LifecycleNode):
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        try:
            ...
            if not self.memory.init(self.domain_file, self.problem_file, self.get_logger()):
    ...
    def generate_plan_custom_solver(self):
        ...
        # Updated regex to handle variable whitespace
        regex_pattern_OPTIC = r"^(\d+\.\d+):\s+\(([^)]+)\)\s+\[(\d+\.\d+)\]$"
        regex_pattern_TFD = r"^(\d+\.\d+):\s\(([^)]+)\)\s\[(\d+\.\d+)\]$"

        if self.solver == "OPTIC":
            regex_pattern = regex_pattern_OPTIC
        elif self.solver == "TFD":
            regex_pattern = regex_pattern_TFD
